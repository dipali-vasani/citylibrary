...
public class SearchResult extends JDialog {
	...
	public SearchResult(String searchKeyword, int searchType, String readerId) {
		...
		if (searchType == SEARCH_TYPE_ID) {
			searchResult = m.execQuery(
					"SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`LIBID`, `POSITION`, `LNAME`, `LLOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`LIBID`=`B`.`LIBID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `D`.`DOCID` = '"
							+ searchKeyword + "';");
		} else if (searchType == SEARCH_TYPE_TITLE) {
			searchResult = m.execQuery(
					"SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`LIBID`, `POSITION`, `LNAME`, `LLOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`LIBID`=`B`.`LIBID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `D`.`TITLE` LIKE '%"
							+ searchKeyword + "%';");
		} else if (searchType == SEARCH_TYPE_PUBLISHER) {
			searchResult = m.execQuery(
					"SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`LIBID`, `POSITION`, `LNAME`, `LLOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`LIBID`=`B`.`LIBID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `P`.`PUBNAME` LIKE '%"
							+ searchKeyword + "%';");
		}
		...
		btnReserve.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				reservedResult = m.execQuery("SELECT * FROM RESERVES R WHERE `R`.`DOCID` = '" + searchArray[rowIndex][0]
						+ "' AND `R`.`COPYNO` = '" + searchArray[rowIndex][5] + "' AND `R`.`LIBID` = '"
						+ searchArray[rowIndex][6] + "';");

				// If this copy dosen't exist in RESERVES table, then check if in the BORROW
				// table and not return (RDTIME IS NULL).
				if (reservedResult == null || reservedResult.size() <= 0) {
					ArrayList<ArrayList<Object>> borrowResult = new ArrayList<ArrayList<Object>>();
					borrowResult = m.execQuery("SELECT * FROM BORROWS B WHERE `B`.`DOCID` = '"
							+ searchArray[rowIndex][0] + "' AND `B`.`COPYNO` = '" + searchArray[rowIndex][5]
							+ "' AND `B`.`LIBID` = '" + searchArray[rowIndex][6] + "' AND `B`.`RDTIME` IS NULL;");
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						...
						for (int q = 0; q < readerReservedArr.length; q++) {
							long dtime = ((Timestamp) readerReservedArr[q][5]).getTime() + 86400000; // Reserved time +
																										// 1 day
							...
							if (cl1.getTimeInMillis() - dtime < 0) {
								...
								if ((System.currentTimeMillis() - cl1.getTimeInMillis()) > 0) {
									...
									if ((dtime - 86400000) - cl1.getTimeInMillis() < 0) {
										int deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DTIME` = '"
												+ readerReservedArr[q][5] + "';");
										if (deletedRows != 1) {
											JOptionPane.showMessageDialog(null, "Failed to remove reservation");
											return;
										}
									} // else if reserved after today 6pm, it isn't expired. don't remove. move to
										// next tuple.
								...
								int deletedRows = m.execUpdate(
										"DELETE FROM RESERVES WHERE `DTIME` = '" + readerReservedArr[q][5] + "';");
								if (deletedRows != 1) {
									JOptionPane.showMessageDialog(null, "Failed to remove reservation");
									return;
								}
						...
						count2 = m.execQuery(
								"SELECT COUNT(*) FROM BORROWS WHERE READERID = '" + readerId + "' AND RDTIME IS NULL;");
						Long count = (Long) (count1.get(0).get(0)) + (Long) (count2.get(0).get(0));

						// If less than 10. then can reserve.
						if (count < 10) {
							int insertedRows = m.execUpdate(
									"INSERT INTO RESERVES(`READERID`, `DOCID`, `COPYNO`, `LIBID`, `DTIME`) VALUES('"
											+ readerId + "','" + searchArray[rowIndex][0] + "','"
											+ searchArray[rowIndex][5] + "','" + searchArray[rowIndex][6]
											+ "', NOW());");
							if (insertedRows != 1) {
					...
					// Check if is himself, then delete the tuple in RESERVE and insert one into
					// RESERVE
					if (((Long) reserveArr[0][1]).toString().equals(readerId)) {
						int deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DOCID` = '"
								+ searchArray[rowIndex][0] + "' AND `COPYNO` = '" + searchArray[rowIndex][5]
								+ "' AND `LIBID` = '" + searchArray[rowIndex][6] + "';");
						if (deletedRows != 1) {
							JOptionPane.showMessageDialog(null, "Failed to remove reservation");
							return;
						}
						...
						int insertedRows = m.execUpdate(
								"INSERT INTO RESERVES(`READERID`, `DOCID`, `COPYNO`, `LIBID`, `DTIME`) VALUES('"
										+ readerId + "','" + searchArray[rowIndex][0] + "','" + searchArray[rowIndex][5]
										+ "','" + searchArray[rowIndex][6] + "', NOW());");
						if (insertedRows != 1) {
							JOptionPane.showMessageDialog(null, "Reserve failed!");
						} else {
							JOptionPane.showMessageDialog(null, "Reserve success!");
						...
						if (cl.getTimeInMillis() - ts < 0) {
							...
							if ((System.currentTimeMillis() - cl.getTimeInMillis()) > 0) {
								...
								if ((ts - 86400000) - cl.getTimeInMillis() < 0) {
									...
									ArrayList<ArrayList<Object>> readerReserved = new ArrayList<ArrayList<Object>>();
									readerReserved = m
											.execQuery("SELECT * FROM RESERVES WHERE `READERID` = '" + readerId + "';");
									Object[][] readerReservedArr = new Object[readerReserved.size()][];
									for (int i = 0; i < readerReserved.size(); i++) {
										ArrayList<Object> row = readerReserved.get(i);
										readerReservedArr[i] = row.toArray();
									...
									for (int q = 0; q < readerReservedArr.length; q++) {
										long dtime = ((Timestamp) readerReservedArr[q][5]).getTime() + 86400000; // Reserved
																													// time
																													// +
																													// 1
																													// day
										...
										if (cl2.getTimeInMillis() - dtime < 0) {
											...
											if ((System.currentTimeMillis() - cl2.getTimeInMillis()) > 0) {
												...
												if ((dtime - 86400000) - cl2.getTimeInMillis() < 0) {
													int deletedRows = m
															.execUpdate("DELETE FROM RESERVES WHERE `DTIME` = '"
																	+ readerReservedArr[q][5] + "';");
													if (deletedRows != 1) {
												...
												} // else if reserved after today 6pm, it isn't expired. don't remove.
													// move to next tuple.
									...
									// Count those reserved by this reader and not expired.
									count1 = m.execQuery(
											"SELECT COUNT(*) FROM RESERVES WHERE READERID = '" + readerId + "';");
									count2 = m.execQuery("SELECT COUNT(*) FROM BORROWS WHERE READERID = '" + readerId
											+ "' AND RDTIME IS NULL;");
									Long count = (Long) (count1.get(0).get(0)) + (Long) (count2.get(0).get(0));

									// If less than 10. then can borrow. delete from RESERVE and insert into
									// RESERVE.
									if (count < 10) {

										int deletedRows = m.execUpdate(
												"DELETE FROM RESERVES WHERE `DOCID` = '" + searchArray[rowIndex][0]
														+ "' AND `COPYNO` = '" + searchArray[rowIndex][5]
														+ "' AND `LIBID` = '" + searchArray[rowIndex][6] + "';");
										if (deletedRows != 1) {
											JOptionPane.showMessageDialog(null, "Failed to remove reservation");
											return;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (reservedResult == null || reservedResult.size() <= 0) {
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						...
						if (count < 10) {
							int insertedRows = m.execUpdate(
									"INSERT INTO BORROWS(`READERID`, `DOCID`, `COPYNO`, `LIBID`, `BDTIME`, `RDTIME`) VALUES('"
											+ readerId + "','" + searchArray[rowIndex][0] + "','"
											+ searchArray[rowIndex][5] + "','" + searchArray[rowIndex][6]
											+ "', NOW(), NULL);");
							if (insertedRows != 1) {
					...
					// Check if is himself, then delete the tuple in RESERVE and insert one into
					// BORROW
					if ((reserveArr[0][1]).toString().equals(readerId)) {
						int deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DOCID` = '"
								+ searchArray[rowIndex][0] + "' AND `COPYNO` = '" + searchArray[rowIndex][5]
								+ "' AND `LIBID` = '" + searchArray[rowIndex][6] + "';");
						if (deletedRows != 1) {
						...
						int insertedRows = m.execUpdate(
								"INSERT INTO BORROWS(`READERID`, `DOCID`, `COPYNO`, `LIBID`, `BDTIME`, `RDTIME`) VALUES('"
										+ readerId + "','" + searchArray[rowIndex][0] + "','" + searchArray[rowIndex][5]
										+ "','" + searchArray[rowIndex][6] + "', NOW(), NULL);");
						if (insertedRows != 1) {
							JOptionPane.showMessageDialog(null, "Borrow failed!");
						} else {
							JOptionPane.showMessageDialog(null, "Borrow success!");
						...
						if (cl.getTimeInMillis() - ts < 0) {
							...
							if ((System.currentTimeMillis() - cl.getTimeInMillis()) > 0) {
								...
								// If now is after 6m and reserved before today 6pm. It is expired. check if
								// more than 10.
								if ((ts - 86400000) - cl.getTimeInMillis() < 0) {
									ArrayList<ArrayList<Object>> count1 = new ArrayList<ArrayList<Object>>();
									ArrayList<ArrayList<Object>> count2 = new ArrayList<ArrayList<Object>>();
